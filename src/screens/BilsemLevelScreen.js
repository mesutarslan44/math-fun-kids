...
import { Brain, Lock, Star, ChevronRight, Trophy } from 'lucide-react-native';
import Layout from '../components/Layout';
import theme from '../constants/theme';
import { getBilsemLevelsWithStatus } from '../utils/BilsemManager';
import { playSelection, playFailure } from '../utils/SoundManager';

const BilsemLevelScreen = ({ navigation }) => {
    const [levels, setLevels] = useState([]);
    const [loading, setLoading] = useState(true);

    const loadLevels = async () => {
        setLoading(true);
        const data = await getBilsemLevelsWithStatus();
        setLevels(data);
        setLoading(false);
    };
    ...
    const handleLevelPress = (level) => {
        ...
        playSelection();
        navigation.navigate('BilsemGame', { levelId: level.id });
    };
        ...
        <Layout>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.titleSection}>
                    <Brain color={theme.colors.purple} size={32} />
                    <Text style={styles.title}>BİLSEM</Text>
                </View>
            ...
            <Text style={styles.subtitle}>🧠 Zeka Soruları</Text>
            <Text style={styles.description}>
                Her seviyeyi geçmek için %70 başarı gerekli
            </Text>
...
export default BilsemLevelScreen;
